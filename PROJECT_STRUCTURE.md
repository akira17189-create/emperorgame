...
### 核心文件（修改游戏逻辑）
1. **`src/engine/policy-engine.ts`** - 政策系统引擎（Phase 3新增）
2. **`src/engine/event-engine.ts`** - 事件系统引擎（Phase 3新增）
3. **`src/engine/idle-engine.ts`** - 放置系统引擎（Phase 3更新）
4. **`src/engine/narrator.ts`** - 叙事生成核心逻辑
5. **`src/engine/arbitration.ts`** - 多Agent仲裁系统
6. **`src/engine/skills.ts`** - 技能系统实现
7. **`src/engine/tick.ts`** - 离线演算引擎
8. **`src/engine/goals-manager.ts`** - NPC目标管理系统
9. **`src/prompts/arbitration.md`** - 仲裁机制文档
10. **`src/prompts/narration.md`** - 叙事提示词模板
